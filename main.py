...
if __name__ == '__main__':
    ...
    parser.add_argument('--number_workers', '-nw', '--num_workers', type=int, default=1)
    ...
    parser.add_argument('--skip_validation', default=True,action='store_true')
    ...
    tools.add_arguments_for_module(parser, datasets, argument_for_class='inference_dataset', default='MpiSintelClean',
                                    skip_params=['is_cropped'],
                                    parameter_defaults={'root': 'C:/Users/CWJ/Desktop/dataset/frames/Pairs_Free_seg10',
                                                        'replicates': 1})
    ...
    with tools.TimerBlock("Parsing Arguments") as block:
        ...
        args.current_hash = subprocess.run(["git", "rev-parse", "HEAD"])
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        gpuargs = {'num_workers': args.effective_number_workers, 
                   'pin_memory': False,
                   'drop_last' : False} if args.cuda else {}
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        if args.save_flow or args.render_validation:
            flow_folder = "{}/inference/Pairs_Free_seg1".format(args.save,args.name.replace('/', '.'),epoch)
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            if args.save_flow or args.render_validation:
                for i in range(args.inference_batch_size):
                    ...
                    idx = batch_idx * args.inference_batch_size + i
                    flo2png.flow2image(_pflow,flow_folder,idx)
                    #flow_utils.writeFlow( join(flow_folder, '%04d.flo'%(batch_idx * args.inference_batch_size + i)),  _pflow)
                    
                    # You can comment out the plt block in visulize_flow_file() for real-time visualization
                    if args.inference_visualize:
                        flow_utils.visulize_flow_file(
                            join(flow_folder, '%04d.flo' % (batch_idx * args.inference_batch_size + i)),flow_vis_folder)
    ...
    for epoch in progress:
        print(((epoch - 1) % args.validation_frequency))
        ...
        if not args.skip_validation and ((epoch - 1) % args.validation_frequency) == 0:
            validation_loss, _ = train(args=args, epoch=epoch - 1, start_iteration=global_iteration, data_loader=validation_loader, model=model_and_loss, optimizer=optimizer,  is_validate=True, offset=offset)
            offset += 1
        ...
        if not args.skip_training:
            train_loss, iterations = train(args=args, epoch=epoch, start_iteration=global_iteration, data_loader=train_loader, model=model_and_loss, optimizer=optimizer, offset=offset)
            global_iteration += iterations
            offset += 1

            # save checkpoint after every validation_frequency number of epochs
            if ((epoch - 1) % args.validat0ion_frequency) == 0:
                checkpoint_progress = tqdm(ncols=100, desc='Saving Checkpoint', position=offset)
                tools.save_checkpoint({   'arch' : args.model,
                                          'epoch': epoch,
                                          'state_dict': model_and_loss.module.model.state_dict(),
