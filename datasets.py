...
def resize_to_target(img):
    # 检查输入图片是否为 [height, width, channel]
    if len(img.shape) != 3 or img.shape[2] != 3:
        raise ValueError("Input image must have 3 dimensions (height, width, channels) with 3 channels for RGB.")
...
class MpiSintel(data.Dataset):
    def __init__(self, args, is_cropped = False, root = './datasets/MPI-Sintel/training/', dstype = 'clean', replicates = 1):
        self.args = args
        ...
        if (self.render_size[0] < 0) or (self.render_size[1] < 0) or (self.frame_size[0]%64) or (self.frame_size[1]%64):
            self.render_size[0] = ( (self.frame_size[0])//64 ) * 16
            self.render_size[1] = ( (self.frame_size[1])//64 ) * 16
        # self.frame_size[0] = resize_to_target(self.frame_size)
        # self.frame_size[1] = resize_to_target(self.frame_size)
